 ...
 * - Fulfulled (이행됨) : 비동기 작업이 성공적으로 완료된 상태
 * - Rejected (거부됨) : 비동기 작업이 실패된 상태
 * 
...
function getCompleteCount() { // 함수 정의
  ...
  // == then( (response) => {return resposne.text()} )
  .then( result => {
...
addBtn.addEventListener("click", () =>{
  ...
  .then( result => {
    // 첫 번째 then에서 반환된 값 중 변환된 text를 result 에 저장
...
const selectTodo = (url) => {
  // 매개변수 url == "/ajax/detail?todoNo=10"
...
changeComplete.addEventListener("click", () => {
  ...
  .then( result => {
    ...
    if(result > 0) { // 성공
      ...
      // update된 DB 데이털르 다시 조회해서 화면에 출력
      // -> 서버 부하가 큼
      // selectTodo();
...
updateView.addEventListener("click", () => {
  ...
  // <button id="updateBtn" data-todo-no="10">수정</button>
 ...
 * 3. DELETE (삭제)
 * - 멱등성 O
 * -> /uesr 사용자 삭제 요청을 여러번 보내도, 첫 요청에서 이미 사용자가 삭제되었다면 이후 요청은 더이상 서버에 영향 주지 않는다.
 * 
 * 4. PUT (갱신)
 * - 멱등성 O
