...
public class MyPageServiceImpl implements MyPageService{
	...
	public int changePw(Map<String, Object> paramMap, int memberNo) {
		...
		String current = (String) paramMap.get("currentPw");
		String newPwConfirm = (String) paramMap.get("newPwConfirm");
		...
		// 다를 경우
		if( !bcrypt.matches(current, originPw) ) {
			return 0;
		} 
		
		// 2. 같을 경우
		// 새 비밀번호를 암호화 (BCryptPasswordEncoder.encode(평문))
		String encPw = bcrypt.encode(newPwConfirm);

		// 진행후 DB에 업데이트
		// SQL 전달해야하는 데이터 2개 (암호화한 새 비밀번호, 회원 번호)
		// -> SQL 전달 인자 1개뿐!
		// -> 묶어서 전달 (paramMap) 
		paramMap.put("encPw", encPw);
		paramMap.put("memberNo", memberNo);
		
		int result = mapper.changePw(paramMap);
	...
	public int fileUpload2(MultipartFile uploadFile, int memberNo) throws Exception {
		...
		// 클라이언트가 파일이 저ㅓ장된 폴더에 접근할 수 있는 주소(정적리소스 요청 주소)
		String webPath = "/myPage/file/";
		
		// 2. DB에 전달할 데이터를 묶어서 INSERT 호출하기
		...
		// 3. 삽입 (INSERT) 성공 시 파일을 저장된 서버 폴더에 저장
		...
		uploadFile.transferTo(new File(folderPath + fileRename));
							// C:/uploadFiles/test/20241112101842_00001.jpg
